...
# Super Decoder - Ultimate Multi-Encoding Decoder for Termux
# SIMPLE DECODER BY ZYUS
# for any error or issue contact at IG/TG-@zyus_9
...
GITHUB_URL="https://github.com/yourusername/super-decoder"
MAX_LAYERS=20
TIMEOUT=30
...
display_banner() {
    ...
    echo "║                                                      ║"
    echo "║      for any error or issue contact at IG/TG-@zyus_9 ║"
    echo "║                                                      ║"
    echo "╚══════════════════════════════════════════════════════╝"
    echo -e "${NC}"
...
# Check dependencies
check_dependencies() {
    local deps=("file" "base64" "xxd" "sed" "grep" "awk" "head" "tail")
    local missing=()
...
create_temp_dir() {
    TEMP_DIR=$(mktemp -d -t super_decoder.XXXXXX)
    echo -e "${BLUE}[i] Temporary directory: $TEMP_DIR${NC}"
}

# Detect all possible encodings in file
detect_encodings() {
    local file="$1"
    local encodings=()
    ...
    # Check for Base64
    if head -c 1000 "$file" 2>/dev/null | grep -q -E '^[A-Za-z0-9+/]*={0,2}$'; then
        encodings+=("base64:90")
    elif head -c 1000 "$file" 2>/dev/null | grep -q -E '^[A-Za-z0-9+/]*=*$'; then
        encodings+=("base64:70")
    fi
    
    # Check for Hex encoding
    if head -c 1000 "$file" 2>/dev/null | grep -q -E '^[0-9A-Fa-f\n\r\t ]*$'; then
        local hex_clean=$(head -c 1000 "$file" | tr -d ' \n\r\t' | grep -E '^[0-9A-Fa-f]*$')
        if [ -n "$hex_clean" ] && [ $(( ${#hex_clean} % 2 )) -eq 0 ]; then
            encodings+=("hex:85")
        fi
    fi
    
    # Check for URL encoding
    if head -c 1000 "$file" 2>/dev/null | grep -q '%[0-9A-Fa-f][0-9A-Fa-f]'; then
        encodings+=("url:80")
    fi
    
    # Check for Binary (raw data)
    if file "$file" | grep -q -E '(binary|data|executable)'; then
        encodings+=("binary:60")
    fi
    ...
    # Check for ROT13
    if head -c 500 "$file" 2>/dev/null | grep -q -E '^[A-Za-z ]*$'; then
        encodings+=("rot13:50")
    fi
    
    # Check for Morse code patterns
    if head -c 500 "$file" 2>/dev/null | grep -q -E '^[\.\- \n\r/t]*$'; then
        encodings+=("morse:40")
    fi
    
    # Check for Base32
    if head -c 1000 "$file" 2>/dev/null | grep -q -E '^[A-Z2-7]*=*$'; then
        encodings+=("base32:75")
    fi
    
    # Check for Reverse text
    local first_line=$(head -1 "$file" 2>/dev/null)
    local reversed=$(echo "$first_line" | rev)
...
decode_hex() {
    local input_file="$1"
    local output_file="$2"
    local temp_hex="$TEMP_DIR/hex_clean.$$"
    
    # Clean hex data (remove spaces, newlines, etc.)
    cat "$input_file" | tr -d ' \n\r\t' | grep -E '^[0-9A-Fa-f]*$' > "$temp_hex"
    
    if [ -s "$temp_hex" ] && [ $(( $(stat -c%s "$temp_hex" 2>/dev/null || echo 0) % 2 )) -eq 0 ]; then
        if xxd -r -p "$temp_hex" > "$output_file" 2>/dev/null; then
            local out_size=$(stat -c%s "$output_file" 2>/dev/null || echo 0)
            if [ "$out_size" -gt 0 ]; then
...
decode_url() {
    local input_file="$1"
    local output_file="$2"
    local temp_url="$TEMP_DIR/url_temp.$$"
    
    # Simple URL decoding using sed
    sed 's/%20/ /g; s/%21/!/g; s/%22/"/g; s/%23/#/g; s/%24/$/g; s/%26/\&/g; s/%27/'"'"'/g; s/%28/(/g; s/%29/)/g; s/%2A/*/g; s/%2B/+/g; s/%2C/,/g; s/%2D/-/g; s/%2E/./g; s/%2F/\//g' "$input_file" | \
    sed 's/%3A/:/g; s/%3B/;/g; s/%3C/</g; s/%3D/=/g; s/%3E/>/g; s/%3F/?/g; s/%40/@/g' | \
    sed 's/%5B/[/g; s/%5C/\\/g; s/%5D/]/g; s/%5E/^/g; s/%5F/_/g; s/%60/`/g' | \
    sed 's/%7B/{/g; s/%7C/|/g; s/%7D/}/g; s/%7E/~/g' > "$temp_url"
    
    # Decode hex characters
    while read -r line; do
        echo "$line" | sed 's/%\([0-9A-Fa-f][0-9A-Fa-f]\)/\\\\x\1/g' | xargs -0 printf 2>/dev/null
    done < "$temp_url" > "$output_file"
...
decode_base32() {
    ...
    if command -v base32 >/dev/null 2>&1; then
        if base32 -d "$input_file" > "$output_file" 2>/dev/null; then
            local out_size=$(stat -c%s "$output_file" 2>/dev/null || echo 0)
            if [ "$out_size" -gt 0 ]; then
                echo -e "${GREEN}[✓] Base32 decoding successful${NC}"
                return 0
            fi
        fi
    fi
...
# Morse code decoder (basic)
decode_morse() {
    local input_file="$1"
    local output_file="$2"
    local temp_morse="$TEMP_DIR/morse_temp.$$"
    
    # Basic Morse code decoding
    cat "$input_file" | tr -d '\n\r' | sed 's/\/ / /g' | sed 's/   / \/ /g' > "$temp_morse"
    
    # Simple Morse to text conversion
    local morse_result=""
    while IFS= read -r -d ' ' word; do
        case "$word" in
            ".-") morse_result="${morse_result}A" ;;
            "-...") morse_result="${morse_result}B" ;;
            "-.-.") morse_result="${morse_result}C" ;;
            "-..") morse_result="${morse_result}D" ;;
            ".") morse_result="${morse_result}E" ;;
            "..-.") morse_result="${morse_result}F" ;;
            "--.") morse_result="${morse_result}G" ;;
            "....") morse_result="${morse_result}H" ;;
            "..") morse_result="${morse_result}I" ;;
            ".---") morse_result="${morse_result}J" ;;
            "-.-") morse_result="${morse_result}K" ;;
            ".-..") morse_result="${morse_result}L" ;;
            "--") morse_result="${morse_result}M" ;;
            "-.") morse_result="${morse_result}N" ;;
            "---") morse_result="${morse_result}O" ;;
            ".--.") morse_result="${morse_result}P" ;;
            "--.-") morse_result="${morse_result}Q" ;;
            ".-.") morse_result="${morse_result}R" ;;
            "...") morse_result="${morse_result}S" ;;
            "-") morse_result="${morse_result}T" ;;
            "..-") morse_result="${morse_result}U" ;;
            "...-") morse_result="${morse_result}V" ;;
            ".--") morse_result="${morse_result}W" ;;
            "-..-") morse_result="${morse_result}X" ;;
            "-.--") morse_result="${morse_result}Y" ;;
            "--..") morse_result="${morse_result}Z" ;;
            ".----") morse_result="${morse_result}1" ;;
            "..---") morse_result="${morse_result}2" ;;
            "...--") morse_result="${morse_result}3" ;;
            "....-") morse_result="${morse_result}4" ;;
            ".....") morse_result="${morse_result}5" ;;
            "-....") morse_result="${morse_result}6" ;;
            "--...") morse_result="${morse_result}7" ;;
            "---..") morse_result="${morse_result}8" ;;
            "----.") morse_result="${morse_result}9" ;;
            "-----") morse_result="${morse_result}0" ;;
            "/") morse_result="${morse_result} " ;;
            *) morse_result="${morse_result}?" ;;
        esac
    done < <(tr ' ' '\n' < "$temp_morse" && echo ' ')
    
    echo "$morse_result" > "$output_file"
    local out_size=$(stat -c%s "$output_file" 2>/dev/null || echo 0)
    
    if [ "$out_size" -gt 0 ] && ! grep -q '^\?*$' "$output_file"; then
        echo -e "${GREEN}[✓] Morse code decoding successful${NC}"
        rm -f "$temp_morse"
        return 0
    fi
...
try_all_decoders() {
    local input_file="$1"
    local output_file="$2"
    local decoders=("base64" "hex" "url" "rot13" "reverse" "base32" "morse" "extract_text")
    
    echo -e "${CYAN}[*] Trying all decoders...${NC}"
    ...
    for decoder in "${decoders[@]}"; do
        ...
        case "$decoder" in
            ...
            "base32") decode_base32 "$input_file" "$temp_output" ;;
            "morse") decode_morse "$input_file" "$temp_output" ;;
            "extract_text") extract_text "$input_file" "$temp_output" ;;
        esac
        
        if [ $? -eq 0 ] && [ -s "$temp_output" ]; then
            cp "$temp_output" "$output_file"
            echo -e "${GREEN}[✓] Success with $decoder decoder${NC}"
            rm -f "$temp_output"
            return 0
        fi
        rm -f "$temp_output"
...
analyze_layers() {
    ...
    while [ $current_layer -lt $MAX_LAYERS ]; do
        ...
        if try_all_decoders "$current_file" "$next_file"; then
            ...
            local current_hash=$(md5sum "$current_file" 2>/dev/null | cut -d' ' -f1)
            local next_hash=$(md5sum "$next_file" 2>/dev/null | cut -d' ' -f1)
            
            if [ "$current_hash" = "$next_hash" ] || [ ! -s "$next_file" ]; then
                echo -e "${YELLOW}[!] No more layers detected${NC}"
                rm -f "$next_file"
                break
            fi
...
show_help() {
    ...
    echo "  • ROT13         • Base32         • Morse Code"
    echo "  • Reverse Text  • Binary Data    • Multi-layer"
    echo ""
    echo "Examples:"
